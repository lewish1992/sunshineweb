...
<div class="page-container">
  ...
  <table class="table table-border table-bordered table-bg">
    ...
    <c:if test="${empty usersEntityList}">
      <div class="alert alert-warning" role="alert">
        <span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>Users表为空，请添加</a>
      </div>
    </c:if>
    <c:if test="${!empty usersEntityList}">
      <tr class="text-c">
      ...
      <tbody>
    <c:forEach items="${usersEntityList}" var="users">
      <tr class="text-c">
        <td><input type="checkbox" value="1" name=""></td>
        <td>${users.userid}</td>
        <td>${users.username}</td>
        <td>${users.password}</td>
        <td>${users.usertypesByUsertypeid.usertypename}</td>
        <td>${users.createtime}</td>
        <td>${users.note}</td>
        <td class="td-status"><span class="label label-success radius">${users.isenabled}</span></td>
